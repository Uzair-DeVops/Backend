...
from ..models.user_model import UserSignUp, UserSignIn, UserResponse, UserCreate, UserCreateWithRoles
...
@router.post("/create-with-roles", response_model=UserResponse, status_code=status.HTTP_201_CREATED)
async def create_user_with_roles_endpoint(
    ...
    - **role_ids**: List of role IDs to assign to this user
    """
    logger.info(f"User creation with roles for email: {user_data.email}")
    return create_user_with_roles(user_data, db)

@router.get("/available-roles", response_model=List[RoleResponse])
async def get_available_roles(db: Session = Depends(get_database_session)):
    """
    Get all available roles for user assignment
    """
    logger.info("Get available roles request")
