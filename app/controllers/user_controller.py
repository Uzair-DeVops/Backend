...
def create_user(user_data: UserSignUp, db: Session) -> UserResponse:
    """Create a new user (simple version without roles)"""
    try:
        ...
        # Create user object with automatic fields
        user = UserCreate(
            email=user_data.email,
            username=user_data.username,
            full_name=user_data.full_name,
            password=hashed_password,
            is_active=True  # Automatically set to active
        )
...
def create_user_with_roles(user_data: UserCreateWithRoles, db: Session) -> UserResponse:
    ...
    try:
        ...
        for role_id in user_data.role_ids:
            try:
                success = assign_role_to_user(user.id, role_id, user.id, db)
                if not success:
                    logger.warning(f"Failed to assign role {role_id} to user {user.username}")
            except Exception as e:
                logger.warning(f"Failed to assign role {role_id} to user {user.username}: {e}")
        
        logger.info(f"User created successfully with roles: {user.email}")
...
def assign_role_to_user(user_id: UUID, role_id: UUID, assigned_by: UUID, db: Session) -> bool:
    """Assign a role to a user"""
    try:
        # Check if role exists
        role = get_role_by_id(role_id, db)
        if not role:
            logger.warning(f"Role assignment failed: role {role_id} not found")
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
        ...
        user = get_user_by_id_internal(user_id, db)
        if not user:
        ...
        existing_assignment = get_user_role(user_id, role_id, db)
...
def get_role_by_id(role_id: UUID, db: Session) -> Optional[Role]:
    """Get role by ID"""
    try:
...
def get_user_role(user_id: UUID, role_id: UUID, db: Session) -> Optional[UserRole]:
...
def get_user_by_id_internal(user_id: UUID, db: Session) -> Optional[UserCreate]:
    """Get user by ID (internal use)"""
    try:
        statement = select(UserCreate).where(UserCreate.id == user_id)
        return db.exec(statement).first()
    except Exception as e:
        logger.error(f"Error getting user by ID: {e}")
        return None 